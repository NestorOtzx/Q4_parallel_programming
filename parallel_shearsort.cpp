...
void sort_row(int** matrix, int n, int row, bool ascending = false) { //O(nlog(n))
...
void sort_column(int** matrix, int n, int col) { // O(nlogn)
    int* aux = new int[n];
    for (int i = 0; i < n; i++) {
        aux[i] = matrix[i][col];
    }
...
void shear_sort(int** matrix, int n) { //O(n^2*log(n)^2)
...
int main() { //O(n^2*log(n)^2)
    ...
    shear_sort(matrix, n); //O(n^2*log(n)^2)

    cout<<"shearsort en paralelo:"<<endl;
    print_matrix(matrix, n);  //O(n^2)

    delete_matrix(matrix, n); //O(n)
    return 0;
}
